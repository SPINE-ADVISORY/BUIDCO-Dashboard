...
CREATE TABLE project (
  ...
  delay_days              INT GENERATED ALWAYS AS (
                            GREATEST(0, CURRENT_DATE::int
                              - COALESCE(revised_end_date, planned_end_date)::int)
                          ) STORED,
  is_delayed              BOOLEAN GENERATED ALWAYS AS (
                            CASE WHEN COALESCE(revised_end_date, planned_end_date) < CURRENT_DATE THEN TRUE
                            ELSE FALSE END
                          ) STORED,
...
CREATE TABLE project_milestone (
  ...
  actual_date    DATE,
  milestone_delay_days INT GENERATED ALWAYS AS (
    CASE WHEN actual_date IS NULL
         THEN GREATEST(0, CURRENT_DATE - planned_date)
         ELSE GREATEST(0, actual_date - planned_date)
    END
  ) STORED,
  created_at     TIMESTAMP DEFAULT NOW(),
  updated_at     TIMESTAMP DEFAULT NOW()
...
BEGIN
  FOREACH col IN ARRAY ARRAY['project_name','status','phase','actual_physical_pct',
    'financial_progress_pct','delay_days','revised_end_date','contractor_id']
  LOOP
...
-- Row Level Security for PMU_ENGINEER scope
ALTER TABLE project ENABLE ROW LEVEL SECURITY;

-- Policy: MD and DC see all rows
...
-- Policy: PMU_ENGINEER sees only their assigned ULBs
...
-- Helper view: project with role-aware edit flag (used by API)
CREATE OR REPLACE VIEW v_project_with_permissions AS
SELECT
  p.*,
  CASE
    WHEN current_setting('app.role', TRUE) = 'MD'           THEN TRUE
    WHEN current_setting('app.role', TRUE) = 'PMU_ENGINEER' THEN TRUE
    ELSE FALSE
  END AS can_edit,
  CASE
    WHEN current_setting('app.role', TRUE) = 'MD' THEN TRUE
...
-- API usage example (set in request context before querying):
-- SET app.role = 'PMU_ENGINEER';
-- SET app.user_id = '42';
-- SELECT * FROM v_project_with_permissions WHERE can_edit = TRUE;
